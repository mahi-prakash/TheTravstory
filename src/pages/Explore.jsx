...
const trips = [
  { id: 1, name: "Paris Summer", location: "Paris, FR" },
  { id: 2, name: "Dubai Luxury", location: "Dubai, UAE" },
  { id: 3, name: "Kyoto Zen", location: "Kyoto, JP" },
];
...
function ExploreFilters({ viewMode, setViewMode }) {
  const [activeTab, setActiveTab] = useState("all");
  const [routeMode, setRouteMode] = useState(null); // 'on-route', 'around', 'near-day'
    ...
    <div className="fixed left-40 top-1/2 -translate-y-1/2 z-30 mt-12 w-[280px]">
      ...
      <div className="relative rounded-[32px] bg-white/90 backdrop-blur-2xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-white/50 overflow-hidden">
        ...
        <div className="px-6 py-5 border-b border-slate-100/50 flex items-center justify-between bg-white/50">
          <h3 className="text-[15px] font-black text-[#0B1527] uppercase tracking-widest">Explore</h3>
              ...
              {mainFilters.map((filter) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all duration-300 group ${activeTab === filter.id
                    ? "bg-[#0B1527] text-white shadow-xl"
                ...
                >
                  ...
                  {activeTab === filter.id && <motion.div layoutId="activeIndReels" className="ml-auto w-1.5 h-1.5 rounded-full bg-sky-400 shadow-[0_0_8px_rgba(56,189,248,0.8)]" />}
                </button>
              ))}
            </div>
          ) : (
                ...
                <div className="space-y-1.5">
                  {categoryFilters.map(f => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-2xl text-[13px] font-bold transition-all duration-300 group ${activeTab === f.id ? "bg-[#0B1527] text-white shadow-xl" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}
                    >
                  ...
                  >
                    {trips.map(trip => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 text-[12px] font-bold transition-all flex items-center gap-2 ${selectedTripId === trip.id ? 'text-[#0081C9] bg-sky-50' : 'text-slate-500 hover:text-[#0B1527] hover:bg-slate-50'
                          }`}
                      >
                        <div className={`w-2 h-2 rounded-full ${selectedTripId === trip.id ? 'bg-[#0081C9]' : 'bg-slate-300'}`} />
                        {trip.name}
                      </button>
                    ))}
                  </motion.div>
      ...
      <div className="mt-4 relative rounded-3xl bg-white/90 backdrop-blur-2xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-white/50 p-2 flex gap-2 overflow-hidden">
        <button
        ...
        >
          ...
          {viewMode === "reels" && (
            <motion.div layoutId="tabInd" className="absolute bottom-0 left-0 right-0 h-1 bg-sky-400 rounded-t-full shadow-[0_-2px_8px_rgba(56,189,248,0.8)]" />
          )}
        </button>
        <button
            ...
            >
              <div className="flex flex-col gap-8">
                ...
                <div className="grid grid-cols-3 gap-6">
                  {reels.concat(reels).map((place, i) => (
                    <motion.div
                      key={i}
                      whileHover={{ y: -8 }}
...
function PlaceDetailView({ place, onClose }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      >
        ...
        <div className="flex-1 h-full overflow-y-auto bg-slate-50 no-scrollbar p-12">
          ...
          <div className="sticky bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border border-white/50 shadow-[0_-20px_40px_rgba(0,0,0,0.05)] rounded-[32px] p-6 flex items-center gap-4">
            ...
            <button className="flex items-center gap-2 px-6 py-4 bg-slate-900 text-white rounded-2xl text-[12px] font-black hover:bg-slate-800 transition-all shadow-xl active:scale-95">
              <Plus size={18} />
              ADD TO DAY 2
            </button>
